...
export async function StartClient(wwclient: Client) {
  ...
  try {
    ...
    for (let chat of document1.BlockedUsers) {
      Groups.push(new MutedUser(chat.groupId, true));

      Groups[Groups.length - 1].muteUserById(chat.users);

      deletedMessage.push(new RevokedMessage(chat.groupId, true));
    }
    //@ts-ignore
    for (let contact of document1.MyContacts) {
      Contacts.push(new SavedContact(contact.contact_serialized, 0));
    }
  } catch (err) {
    let chats: Array<Chat> = await wwclient.getChats();
    let saves: Database = {
      BlockedUsers: chats
        .filter((element) => element.isGroup)
        .map((element: Chat) => {
          return {
      ...
      MyContacts: chats.map((element) => {
        return { contact_serialized: element.id._serialized, cCounter: 0 };
      }),
    };
    database.put({
      _id: "Saves",
    ...
    for (let user of saves.BlockedUsers) {
      Groups.push(new MutedUser(user.groupId, true));
    }
  }
